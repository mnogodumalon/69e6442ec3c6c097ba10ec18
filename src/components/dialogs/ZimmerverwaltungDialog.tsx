...
interface ZimmerverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Zimmerverwaltung['fields']) => Promise<void>;
  defaultValues?: Zimmerverwaltung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function ZimmerverwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: ZimmerverwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<Zimmerverwaltung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'zimmerverwaltung');
      await onSubmit(clean as Zimmerverwaltung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "zimmer_name": string | null, // Zimmername / -nummer\n  "zimmer_typ": LookupValue | null, // Zimmertyp (select one key: "einzelzimmer" | "doppelzimmer" | "suite" | "freigehege") mapping: einzelzimmer=Einzelzimmer, doppelzimmer=Doppelzimmer, suite=Suite, freigehege=Freigehege\n  "kapazitaet": number | null, // Kapazität (Anzahl Katzen)\n  "tagespreis": number | null, // Tagespreis (€)\n  "beschreibung": string | null, // Beschreibung\n  "ausstattung": LookupValue[] | null, // Ausstattung (select one or more keys: "kletterbaum" | "fensterplatz" | "kuschelhoehle" | "spielzeug" | "heizung" | "kamera" | "freigehege_zugang") mapping: kletterbaum=Kletterbaum, fensterplatz=Fensterplatz, kuschelhoehle=Kuschelhöhle, spielzeug=Spielzeug, heizung=Heizung, kamera=Kameraüberwachung, freigehege_zugang=Freigehege-Zugang\n  "zimmer_status": LookupValue | null, // Status (select one key: "verfuegbar" | "belegt" | "reinigung" | "gesperrt") mapping: verfuegbar=Verfügbar, belegt=Belegt, reinigung=In Reinigung, gesperrt=Gesperrt\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Zimmerverwaltung['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, zimmer_foto: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Zimmerverwaltung bearbeiten' : 'Zimmerverwaltung hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="zimmer_foto">Foto des Zimmers</Label>
            {fields.zimmer_foto ? (
              <div className="flex items-center gap-3 rounded-lg border p-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm truncate text-foreground">{fields.zimmer_foto.split("/").pop()}</p>
                  <div className="flex gap-2 mt-1">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            setFields(f => ({ ...f, zimmer_foto: fileUrl }));
                    ...
                    <button
                      ...
                      onClick={() => setFields(f => ({ ...f, zimmer_foto: undefined }))}
                    >
                      Entfernen
                    </button>
                  </div>
